...
class CBType(ABC):
    ...
    @abstractmethod
    def apply_operator(
        self, builder: ir.IRBuilder, lhs: CBValue, op: Token, rhs: CBValue
    ) -> CBValue:
        pass
    ...
    def apply_unary_operator(
        self, builder: ir.IRBuilder, operand: CBValue, op: Token
    ) -> CBValue:
        pass

    @abstractmethod
    def is_compatible_with(self, other: CBType) -> bool:
        pass
...
class UserType(CBType):
    def __init__(self, name: Token, members: dict[str, Member] | None = None) -> None:
        self.user_name = name
        self._members: dict[str, Member] = members if members else {}
    ...
    def apply_operator(
        ...
        raise compile_error(
            op, f"Operator '{op.raw}' not supported by class '{self.name}'."
        )
    ...
    def apply_unary_operator(
        ...
        raise compile_error(
            op, f"Unary operator '{op.raw}' not supported by class '{self.name}'."
        )

    def is_compatible_with(self, other: CBType) -> bool:
        if not isinstance(other, UserType):
            return False
        return self.user_name.raw == other.user_name.raw
...
class IntType(CBType):
    ...
    def apply_unary_operator(
        ...
        if op.raw == "-":
            ...
            raise compile_error(
                op, f"Unary operator '{op.raw}' not supported by 'int' type."
            )
...
class FloatType(CBType):
    ...
    def apply_unary_operator(
        ...
        if op.raw == "-":
            ...
            raise compile_error(
                op, f"Unary operator '{op.raw}' not supported by 'float' type."
            )
    ...
    def apply_operator(
        ...
        if op.raw == "+":
            ...
            raise compile_error(
                op, f"Operator '{op.raw}' not supported by 'float' type."
            )
...
class CharType(CBType):
    ...
    def apply_unary_operator(
        ...
        raise compile_error(
            op, f"Unary operator '{op.raw}' not supported by 'char' type."
        )
    ...
    def apply_operator(
        ...
        if op.raw == "==":
            ...
            raise compile_error(
                op, f"Operator '{op.raw}' not supported by 'char' type."
            )
...
class BoolType(CBType):
    ...
    def apply_unary_operator(
        ...
        if op.raw == "!":
            ...
            raise compile_error(
                op, f"Unary operator '{op.raw}' not supported by 'bool' type."
            )
    ...
    def apply_operator(
        ...
        if op.raw == "==":
            ...
            raise compile_error(
                op, f"Operator '{op.raw}' not supported by 'bool' type."
            )
...
class StringType(CBType):
    ...
    def apply_unary_operator(
        ...
        raise compile_error(
            op, f"Unary operator '{op.raw}' not supported by 'string' type."
        )
