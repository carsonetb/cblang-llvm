...
class Scanner:
    ...
    def scan_token(self):
        ...
        match character:
            ...
            case '[': self.add_token(TokenType.LEFT_CURLY)
            case ']': self.add_token(TokenType.RIGHT_CURLY)
            case ';': self.add_token(TokenType.SEMICOLON)
            case ',': self.add_token(TokenType.COMMA)
            case '.': self.add_token(TokenType.DOT)
            case '+': self.add_token(TokenType.PLUS_EQUAL if self.match('=') else TokenType.PLUS)
            case '^': self.add_token(TokenType.CARET_EQUAL if self.match('=') else TokenType.CARET)
            case '%': self.add_token(TokenType.MODULO if self.match('=') else TokenType.MODULO_EQUAL)
            ...
            case '<': self.add_token(TokenType.LESS_EQUAL if self.match('=') else TokenType.LEFT_ANGLE)
            case '>': self.add_token(TokenType.GREATER_EQUAL if self.match('=') else TokenType.RIGHT_ANGLE)
            case '|':
                if self.match("|"):
    ...
    def add_token(self, ttype: TokenType, literal: int | float | str | bool | None = None):
    ...
    def match(self, character: str, amount: int = 1) -> bool:
